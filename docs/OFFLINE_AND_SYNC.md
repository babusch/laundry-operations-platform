...
JSON contracts use camelCase field names. The current contract is `packages/contracts/events/scan-observed.v2.schema.json`; historical v1 remains readable. Both represent raw observations, so operator, aggregate, causation, and resolved item fields are not applicable until later processing supplies that context.

## Delivery rules

The gateway accepts only `submit-scan.v2` requests on its Development-only `POST /api/scans`, after loopback HTTPS source-cookie authentication, explicit `scans.submit` authorization, and antiforgery validation. The request cannot assert device or location scope. The gateway derives tenant, plant, station, and `sourceId`, assigns gateway time, and commits the immutable `scan.observed.v2` evidence plus a pending outbox row in one plant database transaction before acknowledging local acceptance. An unchanged retry is idempotent only for that same trusted source. Cloud ingestion accepts both v2 and immutable historical v1 events so upgrades cannot strand queued or replayable evidence. Source authentication does not replace the authenticated operator/workflow context required before production scans become business actions. See [ADRs 0014](decisions/0014-authenticate-local-scan-submission.md), [0015](decisions/0015-minimal-trusted-scan-v2-contract.md), [0016](decisions/0016-retire-v1-station-submission.md), and [0017](decisions/0017-activate-trusted-scan-v2.md).

Checkpoint 5.4.5 verifies that this local source authentication and durable acceptance path needs neither cloud nor Keycloak, survives a gateway process restart with the same browser credential, rejects a locally revoked source immediately, and preserves one observation/outbox pair on unchanged retry. Central revocation still reaches an offline gateway only after later configuration synchronization; local plant revocation is immediate.
