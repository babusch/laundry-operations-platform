...
The user approved PostgreSQL for plant storage ([ADR 0005](decisions/0005-use-postgresql-for-plant-storage.md)). Checkpoints 1–2 of the [local gateway plan](LOCAL_GATEWAY_PLAN.md) are implemented: `Laundry.Edge` runs on localhost:5200, with independent `plant-postgres` storage on port 15433 and its own named volume. Health endpoints distinguish process liveness from plant database connectivity; neither depends on cloud availability.

Gateway `POST /api/scans` now validates `submit-scan.v1`, assigns gateway time, and commits immutable observation evidence plus a pending outbox row in one transaction. It is Development/loopback-only and checks a configured synthetic tenant/plant/station/device tuple. Retries retain the original receipt; conflicting IDs return 409. No cloud forwarding runs yet. The `LocalScanAcceptance` migration is applied to the local plant database; startup does not mutate schemas. See [ADR 0006](decisions/0006-durable-local-scan-acceptance.md).

Verification: all 78 .NET tests (34 gateway, 44 cloud) and all 10 Node contract tests pass. The build has zero warnings/errors. Tests cover atomic rollback when outbox insertion fails, retries/concurrent conflicts, tenant/plant/station/device scope, HTTP boundaries, source-clock preservation, restart persistence, and database outage/recovery without a cloud service. A live synthetic gateway submission returned 201 and its unchanged retry returned 200 with the same acceptance timestamp. One synthetic scan remains pending in the development plant database.

Next checkpoint: add background forwarding of the durable outbox to the cloud, with unchanged payload retries and confirmed delivery bookkeeping. Authenticated remote access must be added before connecting across machines. Full workflow authorization, business effects, item resolution, and UI remain pending.
