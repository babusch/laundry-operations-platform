...
The first two reviewed station-application foundation steps are complete. `apps/station-pwa` is an empty React/Vite/TypeScript Development simulator shell with one accessibility-focused render test; it sends no scans and includes no enrollment, service worker, offline queue, or workflow UI. `packages/api-client` supplies an `openapi-fetch` client and generated TypeScript types for the existing v2 scan endpoint from the gateway-owned OpenAPI description. The next station behavior must be selected and reviewed before implementation.
