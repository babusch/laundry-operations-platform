...
Current checkpoint: the Development gateway-to-cloud path is authenticated end to end. The gateway obtains and caches a short-lived Keycloak token, sends its immutable outbox event over local HTTPS, and the cloud validates the token and token-derived tenant/plant scope before durable ingestion. Both application routes remain loopback-only in Development; Staging and Production ingestion remain disabled.

Cloud verification includes 54 tests covering validation, authentication, local-only access, tenant/plant boundaries, concurrent duplicates/conflicts, lost-acknowledgement retries, delayed observations, and database outage/recovery. The initial migration and additive payload-preservation migration are applied to the development database. Docker PostgreSQL uses `localhost:15432` to avoid an existing Windows PostgreSQL service on port 5432. See `docs/DEVELOPMENT_SETUP.md` for repeatable commands.
...
Current verification: all 157 .NET tests pass (102 gateway, 54 cloud, one cross-component scenario). The build succeeds with zero warnings/errors, and the 10 Node contract tests previously passed with unchanged contracts. Tests cover token caching and refresh, concurrent token callers, identity failure classification, scoped diagnostics, pagination, replay idempotency/concurrency, stale state/live lease rejection, audit/requeue rollback, storage outage/recovery, and replay through the gateway/cloud path without payload changes or duplicates.

Checkpoint 5 security is proceeding in reviewed slices. ADR 0009 remains the broader proposed model; its offline policy is approved. Local Keycloak, development gateway identity, cloud validation, and gateway token acquisition are verified (ADRs 0010–0013). Checked-in forwarding still defaults off so ordinary gateway startup requires no secret or cloud; the documented helper enables authenticated forwarding. Secure station-to-gateway access, attributed supervisor replay, production identity hosting, and asymmetric installation credentials remain open.
...
Gateway token acquisition slice complete (ADR 0013): a singleton provider obtains client-credentials tokens, caches them only in memory, serializes refresh, refreshes before expiry, and retries a cloud 401 once with a replacement. Identity failures retain events as pending. Live gateway-to-Keycloak-to-cloud verification synchronized one synthetic observation without printing the credential, token, or identifier. Application processes were stopped afterward; the four local dependency containers remain running. Checked-in forwarding defaults off and is enabled by the safe development helper.
