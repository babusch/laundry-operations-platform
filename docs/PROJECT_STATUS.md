...
Current verification: all 183 .NET tests pass (127 gateway, 55 cloud, one cross-component scenario), and all 10 Node contract tests pass with unchanged schemas. Live trusted-HTTPS proofs cover enrollment/protected acceptance and checkpoint 5.4.5 restart/revocation resilience. The latest helper used the same source cookie after a real gateway restart, returned the original observation on unchanged retry, immediately rejected a locally revoked source, confirmed the rejected event was absent, stopped the gateway, and removed temporary logs. It disclosed no credential, antiforgery token, or tag value. The build succeeds with zero warnings/errors. Tests cover WAN-independent local acceptance, same-credential restart, local revocation, cross-scope copied credentials, invalid cookies/CSRF, authenticated durable acceptance, arbitrary untrusted v1 device metadata, enrollment, credential lifecycle, source isolation, gateway token resilience, diagnostics, replay, storage outage/recovery, and end-to-end gateway/cloud delivery without payload changes or duplicates.

Checkpoint 5 security is proceeding in reviewed slices. ADR 0009 remains the broader proposed model; its offline policy is approved. Local Keycloak, development gateway identity, cloud validation, gateway token acquisition, identity outages, disabled identities, signing-key rollover, protected local scan acceptance, and its offline/restart/revocation behavior are verified (ADRs 0010–0014). Checkpoints 5.4.1–5.4.5 are complete. No physical-device table or device-based authorization was added. The browser cookie is explicitly a same-gateway bearer credential; stronger copy resistance requires a selected device-bound mechanism. Authenticated operator/workflow context is required before scans count as laundry business actions, but its design remains deferred for joint review. Checkpoint 5.4.6 is the first real-adapter proof and cannot be selected responsibly until the first hardware and adapter deployment model are known. Attributed supervisor replay, production identity hosting, asymmetric installation credentials, enrollment recovery, and production credential lifetimes also remain open.
