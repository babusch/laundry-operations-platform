...
Current verification: all 183 .NET tests pass (127 gateway, 55 cloud, one cross-component scenario), and all 20 Node contract tests pass. Strict v2 request/event schemas and synthetic barcode/RFID examples are verified alongside unchanged v1. Live trusted-HTTPS proofs cover enrollment/protected acceptance and checkpoint 5.4.5 restart/revocation resilience. The latest helper used the same source cookie after a real gateway restart, returned the original observation on unchanged retry, immediately rejected a locally revoked source, confirmed the rejected event was absent, stopped the gateway, and removed temporary logs. It disclosed no credential, antiforgery token, or tag value. The build succeeds with zero warnings/errors.

Checkpoint 5 security is proceeding in reviewed slices. ADR 0009 remains the broader proposed model; its offline policy is approved. Local Keycloak, development gateway identity, cloud validation, gateway token acquisition, protected local scan acceptance, and its resilience behavior are verified (ADRs 0010–0014). Checkpoints 5.4.1–5.4.5 are complete; the real-adapter proof remains deferred until hardware is available. ADR 0015 now defines the contract-only v2 boundary: the station submits minimal observation data, and the gateway adds trusted tenant/plant/station/source attribution. No runtime accepts v2 yet. The next small slice is parallel gateway v2 validation and enrichment while retaining v1 compatibility. Authenticated operator/workflow context remains required before scans count as laundry business actions.
