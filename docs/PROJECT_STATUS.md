...
The first three reviewed station-application foundation steps are complete. `apps/station-pwa` is a React/Vite/TypeScript Development simulator shell that reports gateway readiness as checking, ready, or unavailable and offers a manual retry after failure. "Ready" is deliberately limited to local plant-storage connectivity and does not imply cloud connectivity or scan acceptance. Two UI tests cover honest success/failure language and retry behavior. It sends no scans and includes no enrollment, service worker, offline queue, or workflow UI. `packages/api-client` supplies an `openapi-fetch` client and generated TypeScript types for the gateway health and v2 scan endpoints from the gateway-owned OpenAPI description. The next station behavior must be selected and reviewed before implementation.
